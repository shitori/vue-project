...
<template>
    <div class="box mt-6 has-text-centered">
        ...
        <button class="button is-info is-outlined is-medium m-1" @click="resetComponentVisibility()">Nouvelle itération
        </button>
        <pre class="m-5 has-text-left">{{ store.moreInfo }}</pre>

        <div class="columns is-multiline has-text-left is-centered ">
            <div class="card m-1 column is-one-quarter " style="width:  20rem;" v-for="result in store.result" :class="{
                ...
                <a target="_blank" class="is-link"
                    :href="'https://testmanagement.factory.orange-business.com/squash/test-case-workspace/test-case/' + result.realId + '/content'">
                    <div class="card-content">
                        ...
                        <div class="content">
                            <p class="has-text-dark">{{ removeHTMLtagsAndShorter(result.extraInfo) }}</p>
                        </div>
